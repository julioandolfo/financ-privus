...
$d = !empty($old) ? $old : $despesa;
$dias = range(1, 31);
$meses = [1 => 'Janeiro', 2 => 'Fevereiro', 3 => 'Março', 4 => 'Abril', 5 => 'Maio', 6 => 'Junho',
    7 => 'Julho', 8 => 'Agosto', 9 => 'Setembro', 10 => 'Outubro', 11 => 'Novembro', 12 => 'Dezembro'];
$rateiosIniciais = [];
if (!empty($despesa['rateios_json'])) {
    $decoded = json_decode($despesa['rateios_json'], true);
    if (is_array($decoded)) {
...
?>
<div class="container mx-auto max-w-5xl" x-data="despesaRecorrenteFormEdit()">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Editar Despesa Recorrente</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1"><?= htmlspecialchars($despesa['descricao']) ?></p>
        </div>
        <a href="/despesas-recorrentes/<?= $despesa['id'] ?>" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">← Voltar</a>
    </div>
    <?php if (!empty($errors)): ?>
    ...
    <?php endif; ?>
    <form method="POST" action="/despesas-recorrentes/<?= $despesa['id'] ?>">
        <input type="hidden" name="_method" value="PUT">
        <?php $old = $d; $empresaId = $despesa['empresa_id'] ?? null; include __DIR__ . '/_form_body.php'; ?>
        <div class="flex justify-end space-x-4 mt-6">
            <a href="/despesas-recorrentes/<?= $despesa['id'] ?>" class="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-xl">Cancelar</a>
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-red-600 to-rose-600 text-white rounded-xl font-medium">Atualizar Despesa Recorrente</button>
        </div>
    </form>
</div>
<script>
function despesaRecorrenteFormEdit() {
    const rateiosIniciais = <?= json_encode($rateiosIniciais) ?>;
    const d = <?= json_encode($despesa) ?>;
    return {
