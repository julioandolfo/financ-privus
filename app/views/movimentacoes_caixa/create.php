<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center space-x-3 text-sm text-gray-600 dark:text-gray-400 mb-4">
            ...
            <span class="text-gray-900 dark:text-gray-100 font-medium">Nova Movimentação</span>
        </div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Nova Movimentação de Caixa</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Registre uma nova movimentação manual (entrada ou saída)</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
        <form method="POST" action="/movimentacoes-caixa">
            <!-- Tipo (Destaque) -->
            <div class="mb-8">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="relative flex items-center justify-center p-6 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-2 border-green-300 dark:border-green-700 rounded-xl cursor-pointer hover:shadow-lg transition-all group">
                        <input type="radio" name="tipo" value="entrada" required
                               class="sr-only peer"
                               <?= ($this->session->get('old')['tipo'] ?? '') === 'entrada' ? 'checked' : '' ?>>
                    ...
                    <label class="relative flex items-center justify-center p-6 bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 border-2 border-red-300 dark:border-red-700 rounded-xl cursor-pointer hover:shadow-lg transition-all group">
                        <input type="radio" name="tipo" value="saida" required
                               class="sr-only peer"
                               <?= ($this->session->get('old')['tipo'] ?? '') === 'saida' ? 'checked' : '' ?>>
                        <div class="text-center peer-checked:scale-105 transition-transform">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="categoria_id" required
                        ...
                        <?php foreach ($categorias as $categoria): ?>
                            <option value="<?= $categoria['id'] ?>" <?= ($this->session->get('old')['categoria_id'] ?? '') == $categoria['id'] ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select name="centro_custo_id"
                        ...
                        <?php foreach ($centrosCusto as $centro): ?>
                            <option value="<?= $centro['id'] ?>" <?= ($this->session->get('old')['centro_custo_id'] ?? '') == $centro['id'] ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select name="forma_pagamento_id"
                        ...
                        <?php foreach ($formasPagamento as $forma): ?>
                            <option value="<?= $forma['id'] ?>" <?= ($this->session->get('old')['forma_pagamento_id'] ?? '') == $forma['id'] ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <input type="number" name="valor" step="0.01" min="0.01" required
                           value="<?= htmlspecialchars($this->session->get('old')['valor'] ?? '') ?>"
                           placeholder="0,00"
                ...
                <div>
                    ...
                    <input type="date" name="data_movimentacao" required
                           value="<?= htmlspecialchars($this->session->get('old')['data_movimentacao'] ?? date('Y-m-d')) ?>"
                ...
                <div>
                    ...
                    <input type="date" name="data_competencia"
                           value="<?= htmlspecialchars($this->session->get('old')['data_competencia'] ?? '') ?>"
                           class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div class="mt-6">
                ...
                <textarea name="descricao" rows="3" required
                          ...
                          class="w-full px-4 py-3 rounded-xl border <?= isset($this->session->get('errors')['descricao']) ? 'border-red-500' : 'border-gray-300 dark:border-gray-600' ?> bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"><?= htmlspecialchars($this->session->get('old')['descricao'] ?? '') ?></textarea>
            ...
            <div class="mt-6">
                ...
                <textarea name="observacoes" rows="2"
                          ...
                          class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"><?= htmlspecialchars($this->session->get('old')['observacoes'] ?? '') ?></textarea>
            </div>
