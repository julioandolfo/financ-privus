<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-gray-700">
        <form method="POST" action="<?= baseUrl('/centros-custo') ?>" class="space-y-6">
            <!-- Empresa -->
            <div>
            ...
            <div>
                ...
                <select id="centro_pai_id" 
                    ...
                    <?php foreach ($centrosPai as $centroPai): ?>
                        <option value="<?= $centroPai['id'] ?>" <?= (($session->get('old')['centro_pai_id'] ?? '') == $centroPai['id']) ? 'selected' : '' ?>>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    empresaSelect.addEventListener('change', function() {
        ...
        fetch(`<?= baseUrl('/centros-custo') ?>?empresa_id=${empresaId}&ajax=1`)
