    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Atualize os dados da categoria</p>
            </div>
        </div>
    </div>

    <!-- Formulário -->
    <form method="POST" action="<?= $this->baseUrl('/categorias-produtos/' . $categoria['id']) ?>" class="space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                       name="nome" 
                       value="<?= htmlspecialchars($this->session->get('old')['nome'] ?? $categoria['nome']) ?>"
                       required
                       maxlength="100"
            ...
            <div class="mb-6">
                ...
                <select name="categoria_pai_id" 
                    ...
                    <?php foreach ($categorias as $cat): ?>
                        <option value="<?= $cat['id'] ?>" 
                                <?= (($this->session->get('old')['categoria_pai_id'] ?? $categoria['categoria_pai_id']) == $cat['id']) ? 'selected' : '' ?>>
            ...
            <div class="mb-6">
                ...
                <textarea name="descricao" 
                          rows="3"
                          class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"><?= htmlspecialchars($this->session->get('old')['descricao'] ?? $categoria['descricao'] ?? '') ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="icone" 
                           value="<?= htmlspecialchars($this->session->get('old')['icone'] ?? $categoria['icone'] ?? '') ?>"
                           maxlength="50"
                           placeholder="Ex: 📦 🏷️ 📱"
                           class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <input type="color" 
                           name="cor" 
                           value="<?= htmlspecialchars($this->session->get('old')['cor'] ?? $categoria['cor'] ?? '#3B82F6') ?>"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           name="ordem" 
                           value="<?= htmlspecialchars($this->session->get('old')['ordem'] ?? $categoria['ordem'] ?? '0') ?>"
                           min="0"
                           class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <select name="ativo" 
                            class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="1" <?= (($this->session->get('old')['ativo'] ?? $categoria['ativo']) == '1') ? 'selected' : '' ?>>Ativo</option>
                        <option value="0" <?= (($this->session->get('old')['ativo'] ?? $categoria['ativo']) == '0') ? 'selected' : '' ?>>Inativo</option>
                    </select>
                </div>
            </div>
