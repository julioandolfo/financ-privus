<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center space-x-4 mb-4">
            ...
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Nova Forma de Pagamento</h1>
        </div>
        <p class="text-gray-600 dark:text-gray-400">Preencha os dados para cadastrar uma nova forma de pagamento</p>
    </div>

    <!-- Formulário -->
    <form method="POST" action="<?= $this->baseUrl('/formas-pagamento') ?>" class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-gray-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select id="tipo" 
                    ...
                    <option value="ambos" <?= ($this->session->get('old')['tipo'] ?? 'ambos') == 'ambos' ? 'selected' : '' ?>>Ambos (Pagamento e Recebimento)</option>
                    <option value="pagamento" <?= ($this->session->get('old')['tipo'] ?? '') == 'pagamento' ? 'selected' : '' ?>>Pagamento</option>
                    <option value="recebimento" <?= ($this->session->get('old')['tipo'] ?? '') == 'recebimento' ? 'selected' : '' ?>>Recebimento</option>
                </select>
