                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-6 mt-3">
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="radio" name="tipo_custo" value="variavel" <?= ($old['tipo_custo'] ?? $conta['tipo_custo'] ?? 'fixo') == 'variavel' ? 'checked' : '' ?>
                                       class="w-5 h-5 text-blue-600 focus:ring-2 focus:ring-blue-500">
                                <span class="text-gray-700 dark:text-gray-300">Variável</span>
                            </label>
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="radio" name="tipo_custo" value="fixo" <?= ($old['tipo_custo'] ?? $conta['tipo_custo'] ?? 'fixo') == 'fixo' ? 'checked' : '' ?>
...
<div id="modalExcluirConta" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6">
        <div class="flex items-center space-x-3 mb-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Excluir Conta a Pagar</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">Esta acao pode ser revertida em Registros Deletados</p>
            </div>
        </div>
        <form method="POST" action="/contas-pagar/<?= $conta['id'] ?>/delete">
            <div class="mb-4">
