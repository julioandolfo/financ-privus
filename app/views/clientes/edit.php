<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-gray-700">
        <form method="POST" action="<?= $this->baseUrl('/clientes/' . $cliente['id']) ?>" class="space-y-6">
            <?php
            $old = $this->session->get('old') ?? [];
            $endereco = $old['endereco'] ?? $cliente['endereco'] ?? [];
            ?>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all <?= ($old['tipo'] ?? $cliente['tipo']) === 'fisica' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-700' ?>">
                        <input type="radio" 
                               name="tipo" 
                               value="fisica" 
                               <?= ($old['tipo'] ?? $cliente['tipo']) === 'fisica' ? 'checked' : '' ?>
                    ...
                    <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all <?= ($old['tipo'] ?? $cliente['tipo']) === 'juridica' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-700' ?>">
                        <input type="radio" 
                               name="tipo" 
                               value="juridica" 
                               <?= ($old['tipo'] ?? $cliente['tipo']) === 'juridica' ? 'checked' : '' ?>
                               class="w-5 h-5 text-blue-600 focus:ring-blue-500" 
                               required>
                        <div class="ml-3">
            ...
            <div>
                ...
                <?php
                $cpfCnpj = $old['cpf_cnpj'] ?? $cliente['cpf_cnpj'] ?? '';
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="email" 
                           id="email" 
                           name="email" 
                           value="<?= htmlspecialchars($old['email'] ?? $cliente['email'] ?? '') ?>"
