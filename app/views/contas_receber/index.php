...
<div class="max-w-7xl mx-auto animate-fade-in">
        ...
        <a href="/transacoes-pendentes" class="flex items-center justify-between bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 border-2 border-amber-300 dark:border-amber-700 rounded-xl p-4 hover:border-amber-400 dark:hover:border-amber-600 transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-700 dark:text-amber-300">Clique para revisar e lançar como contas a receber</p>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden" 
         x-data="{ selecionados: [], showCategoriaModal: false, showBaixaModal: false, showEditarDataModal: false, showDeleteModal: false, tipoData: 'manual', tipoBaixaData: 'manual', motivoExclusao: 'Exclusão em massa pelo usuário' }"
        ...
        <!-- Modal Editar Data de Recebimento em Massa -->
        <div x-show="showEditarDataModal" x-cloak
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-md w-full">
                <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-1">Editar Data de Recebimento</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Atualizar a data de recebimento das contas selecionadas.</p>
                <form method="POST" action="/contas-receber/atualizar-data-massa">
                    <?php
                    $urlParams = array_filter($filters ?? [], fn($v) => $v !== '' && $v !== null);
                    foreach ($urlParams as $k => $v):
                        if (!is_array($v)):
                    ...
                    <div class="space-y-4 mb-6">
                        <div class="flex flex-col gap-3">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer p-3 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                                   :class="tipoData === 'manual' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                                <input type="radio" x-model="tipoData" value="manual" class="text-green-600 focus:ring-green-500">
                                <span class="text-sm text-gray-700 dark:text-gray-300 font-medium">Inserir data manualmente</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer p-3 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                                   :class="tipoData === 'vencimento' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : ''">
                                <input type="radio" x-model="tipoData" value="vencimento" class="text-green-600 focus:ring-green-500">
                        ...
                        <div x-show="tipoData === 'manual'" x-transition>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Data de Recebimento</label>
                            <input type="date" name="data_recebimento_massa" :required="tipoData === 'manual'" value="<?= date('Y-m-d') ?>"
                                   class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                        </div>
                    </div>
                    <div class="flex justify-end gap-2">
        ...
        <div x-show="showBaixaModal" x-cloak
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-md w-full">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Receber as contas selecionadas. Contas com parcelas ou já recebidas serão ignoradas.</p>
                <form method="POST" action="/contas-receber/efetuar-baixa-massa">
                    <?php
                    $urlParams = array_filter($filters ?? [], fn($v) => $v !== '' && $v !== null);
                    foreach ($urlParams as $k => $v):
                        if (!is_array($v)):
                    ...
                    <input type="hidden" name="tipo_data_baixa" :value="tipoBaixaData">
                    <div class="space-y-4 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Data Recebimento</label>
                            <div class="flex gap-2 mb-2">
                                <label class="flex-1 flex items-center gap-2 cursor-pointer p-2.5 rounded-lg border transition-colors"
                                       :class="tipoBaixaData === 'manual' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'">
                                    <input type="radio" x-model="tipoBaixaData" value="manual" class="text-green-600 focus:ring-green-500">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">Data manual</span>
                                </label>
                                <label class="flex-1 flex items-center gap-2 cursor-pointer p-2.5 rounded-lg border transition-colors"
                                       :class="tipoBaixaData === 'vencimento' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'">
                                    <input type="radio" x-model="tipoBaixaData" value="vencimento" class="text-green-600 focus:ring-green-500">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">Data de vencimento</span>
                                </label>
                            </div>
                            <div x-show="tipoBaixaData === 'manual'" x-transition>
                                <input type="date" name="data_recebimento" :required="tipoBaixaData === 'manual'" value="<?= date('Y-m-d') ?>"
                                       class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                            </div>
                            <div x-show="tipoBaixaData === 'vencimento'" x-transition>
                                <p class="text-sm text-gray-500 dark:text-gray-400 italic px-1">Cada conta será recebida na sua própria data de vencimento.</p>
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Forma de Recebimento</label>
                            <select name="forma_recebimento_id" required class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                                <option value="">Selecione...</option>
                                <?php foreach ($formasRecebimento ?? [] as $fr): ?>
                                <option value="<?= $fr['id'] ?>"><?= htmlspecialchars($fr['nome']) ?><?= !empty($fr['empresa_nome']) ? ' (' . htmlspecialchars($fr['empresa_nome']) . ')' : '' ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="showBaixaModal = false"
                                class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Cancelar</button>
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">Dar Baixa</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            $urlBase = '/contas-receber' . (!empty($urlParams) ? '?' . http_build_query($urlParams) : '');
